...
class EmersonValeSupabaseUser extends BaseAuthUser {
  EmersonValeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> emersonValeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EmersonValeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
